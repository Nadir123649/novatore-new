...
import { sericesoffer } from "../constants/indesx";
...
const CloudOps: React.FC = () => {
  ...
  const { digitalservice, cybersecurity } = sericesoffer;
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (ulRef.current) {
        ...
        const middleIndex = Math.floor((scrollTop + clientHeight / 10) / itemHeight);
              ...
              <Col lg={12} md={12} xs={12}>
                <div className="grid grid-cols-12 gap-4 py-[20px] min-h-[100vh]">
                  ...
                  <div className="col-span-8">
                    <div
                      className="overflow-y-scroll h-[700px] parallax-section relative"
                      ref={ulRef}
                    >
                      <ul className="pr-[40px] flex flex-col gap-10">
                        {duplicatedDetails.map((data, index) => (
                          <li
                            key={index}
                            className={`relative h-[200px] py-10 transition-all duration-300 ${index === activeIndex
