...
const DataServicesSection: React.FC<DataServicesSectionProps> = ({ heading, headingBlue, data, needsMoreTitle, type }) => {
                            ...
                            data.map((item, index) => (
                                <Col lg={6} md={6} xs={12} key={index} className="mb-4">
                                    <DataCard title={item.title} description={item.description} icon={item.icon} listItems={item.listItems}
                                        type="relative bg-white shadow rounded-[16px] pt-[40px] pb-[10px] px-[24px]  max-w-2xl mx-auto overflow-hidden min-h-[370px] flex flex-col h-full hover:scale-105 transition ease-in duration-1.5"
